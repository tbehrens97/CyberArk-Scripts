#Created by Trevor Behrens Feb 2025
...
#Gets the whole Path #C:\Users\T1ADM-CyberArk08\Desktop\Privileged Access Manager Self-Hosted_14.4_1738004326579\Privileged Session Manager\Privileged Session Manager-Rls-v14.4.zip
...
##########################
$VaultIP = '"vaultip" Value="X.X.X.X,Y.Y.Y.Y"'
##########################
...
Invoke-Command -Session $Session -ScriptBlock {$filePath = "C:\Program Files (x86)\CyberArk\PSM\Hardening\AppLockerArchive\PSMConfigureAppLocker_$CurrentDay.xml"
if (Test-Path $filePath) {} else {Copy-Item "C:\Program Files (x86)\CyberArk\PSM\Hardening\PSMConfigureAppLocker.xml" -Destination "C:\Program Files (x86)\CyberArk\PSM\Hardening\AppLockerArchive\PSMConfigureAppLocker_$CurrentDay.xml" -Force}}

echo "Copying $outputFile to $Server"

robocopy $directoryPath "\\$server\C$\Users\$env:UserName\Desktop" "$outputFile" /COPY:DAT /R:3 /W:5
#Copy-Item "$FileName" -Destination "\\$server\C$\Users\$env:UserName\Desktop\$outputFile" -Force #Old way to copy the file
...
$command = {Invoke-Command -Session $Session -ScriptBlock {.\Execute-Stage.ps1 .\Installation\InstallationConfig.XML silent -modeOfOperation Upgrade}}
...
# Check if less than 30 seconds
if ($stopwatch.Elapsed.TotalSeconds -lt 30) {
    start-sleep 10
    Write-Host "Command finished too quickly. Rerunning..."
    & $command
}
...
Copy-Item "C:\Software\dotnet-core-uninstall-1.7.656206.msi" -Destination "\\$server\C$\dotnet-core-uninstall-1.7.656206.msi"
Echo "Running dotnet-core-uninstall"
Invoke-Command -Session $Session -ScriptBlock {
$MSI="C:\dotnet-core-uninstall-1.7.656206.msi"
...
################
$NewDotNet="C:\Software\dotnet-hosting-8.0.22-win.exe"
################
...
echo "$Server does not need AutoShareServer value to be updated"
}

Start-Sleep 5
#Allow any Certificate from Server
Add-Type @"
using System.Net;
...
#Check to make sure Health Page Is showing PASS
$HealthCheck= Invoke-WebRequest "https://$Server/psm/api/health" -UseBasicParsing

if ($HealthCheck.Content -ne "PASS"){
Write-Host "$Server Health page came back with not PASS" -BackgroundColor Red
...
Else{
Write-Host "$Server Health page came back with PASS" -BackgroundColor Green
}
}
Else{
Write-Host "$Server Health page came back with PASS" -BackgroundColor Green
