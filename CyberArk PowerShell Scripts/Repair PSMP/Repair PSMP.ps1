...
foreach ($Server in $ActiveServers) {
    ...
    Set-SCPItem -ComputerName $Server -Path C:\Software\RPM-GPG-KEY-CyberArk -Destination "/home/$usernamelower" -Credential $UserCred -AcceptKey -ConnectionTimeout 60
    ...
    Invoke-SSHCommand -SSHSession $Session -Command "sudo rpm -Uvh --force ~/$FileNameNoExten/$CARK_Version" -ShowStandardOutputStream -TimeOut 360
